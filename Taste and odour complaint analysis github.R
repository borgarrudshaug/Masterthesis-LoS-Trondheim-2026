...
config <- list(
  ...
  event_basins      = c("Trollahaugen_HB", "Huseby_HB"),
...
combine_herlofsenloypa <- function(df) {
  ...
  df |> mutate(
    nearest_basin_id = case_when(
      nearest_basin_id_raw %in% c("Herlofsenloypa_indre_HB",
                                  "Herlofsenloypa_ytre_HB") ~ "Herlofsenloypa_HB",
...
read_lookup_tables <- function(path) {
  ...
  basin_name_lookup <- tibble::tribble(
    ~basin_id,                     ~source_name,
    "Torshaug_HB",                 "Torshaug hb",
    "Huseby_HB",                   "Huseby hb",
    "Reppesaasen_HB",              "Reppes\u00E5sen hb",
    "Trollahaugen_HB",             "Trollahaugen hb",
    "Hogaasen_HB",                 "H\u00F8g\u00E5sen hb basseng 1",
    "Reinaasen_HB",                "Rein\u00E5sen hb",
    "Lauvaasen_HB",                "Lauv\u00E5sen hb",
    "Steinan_HB",                  "Steinan hb",
    "Kuhaugen_HB",                 "Kuhaugen hb basseng 1",
    "Granaas_HB",                  "Gran\u00E5sen hb",
    "Sagbergkammen_indre_HB",      "Sagbergkammen hb",
    "Sagbergkammen_ytre_HB",       "Sagbergkammen hb",
    "Herlofsenloypa_HB",           "Herlofsenl\u00F8ypa hb",
    "Herlofsenloypa_indre_HB",     "Herlofsenl\u00F8ypa hb",
    "Herlofsenloypa_ytre_HB",      "Herlofsenl\u00F8ypa hb",
    "Grostadaunet_ytre_HB",        "Grostadaunet hb",
    "Grostadaunet_indre_HB",       "Grostadaunet hb"
  )
...
complaints_df <- complaints_df |>
  ...
  flag_event_period(config$event_start, config$event_end, config$event_basins) |>
...
junctions_per_basin <- tibble(
  ...
  mutate(nearest_basin_id = case_when(
    str_detect(nearest_basin_id, "(?i)herlofsenloypa|herlofsenl\u00F8ypa") ~ "Herlofsenloypa_HB",
...
complaint_rate <- complaints_baseline |>
  ...
  mutate(complaints_per_1000 = n_complaints / n_junctions * 1000) |>
...
p_basin <- ggplot(basin_complaints, aes(x = reorder(nearest_basin_id, n_complaints), y = n_complaints)) +
  geom_col(fill = "#3b82f6", alpha = 0.85) + coord_flip() +
  scale_y_continuous(breaks = scales::breaks_pretty(n = 10)) +
  labs(x = "Nearest basin", y = "Number of complaints") +
  theme_bw()
ggsave(file.path(output_dir, "figures", "complaints_by_basin_baseline.png"), p_basin, width = 9, height = 6, dpi = 600)

p_map <- ggplot() +
  geom_sf(data = basins$sf, size = 3, colour = "#ef4444") +
  geom_sf(data = complaints$sf, size = 1.5, alpha = 0.5, colour = "#3b82f6") +
  labs(title = "Complaint locations and service reservoirs") +
  theme_bw()
ggsave(file.path(output_dir, "figures", "complaint_locations_and_basins.png"), p_map, width = 8, height = 7, dpi = 300)
...
p_map <- ggplot() +
  geom_sf(data = basins$sf, size = 3, colour = "#ef4444") +
  geom_sf(data = complaints$sf, size = 1.8, alpha = 0.55, colour = "#3b82f6") +
  coord_sf(xlim = map_xlim, ylim = map_ylim, expand = FALSE) +
  labs(title = "Complaint locations and service reservoirs") +
  theme_bw()
ggsave(file.path(output_dir, "figures", "complaint_locations_and_basins.png"),
       p_map, width = 6, height = 9, dpi = 300)

# ---- Complaint map coloured by water age ----
complaints_map_df <- complaints_baseline |>
  filter(!is.na(water_age_mean), !is.na(lat), !is.na(lon))

complaints_map_sf <- sf::st_as_sf(complaints_map_df,
                                  coords = c("lon", "lat"),
                                  crs = 4326) |>
  sf::st_transform(sf::st_crs(complaints$sf))

p_age_map <- ggplot() +
  geom_sf(data = complaints_map_sf, aes(colour = water_age_mean),
          size = 2.4, alpha = 0.85) +
  scale_colour_viridis_c(option = "inferno", name = "Water age\n(hours)") +
  coord_sf(xlim = map_xlim, ylim = map_ylim, expand = FALSE) +
  labs(title = "Complaint locations coloured by water age") +
  theme_bw()
ggsave(file.path(output_dir, "figures", "complaint_map_water_age.png"),
       p_age_map, width = 6, height = 9, dpi = 300)

# ---- Mix index vs complaints for three key basins ----

focus_basins <- c("Torshaug_HB", "Huseby_HB", "Herlofsenloypa_HB")
...
complaints_monthly_basin <- complaints_df |>
  ...
  rename(basin_label = nearest_basin_id)
...
group_sizes <- map_dfr(
  ...
  function(v) {
    data_ord |>
      ...
      select(variable, group, n, included_in_PERMANOVA)
  }
)
...
p_bi <- ggplot() +
  geom_point(data = pca_scores, aes(x = PC1, y = PC2, colour = nearest_basin_id),
             size = 2, alpha = 0.7) +
  ...
  geom_text_repel(data = loadings_for_plot,
                  aes(x = PC1s, y = PC2s, label = variable), size = 3.2, colour = "grey20") +
  labs(title = "PCA biplot by nearest basin",
       x = paste0("PC1 (", round(pca_var[1], 1), "%)"),
       y = paste0("PC2 (", round(pca_var[2], 1), "%)"),
       colour = "Nearest basin") +
  theme_bw()
...
p_nmds_basin <- ggplot(nmds_scores, aes(x = NMDS1, y = NMDS2, colour = nearest_basin_id)) +
  geom_point(size = 2, alpha = 0.7) +
  labs(title = "NMDS by nearest basin", subtitle = nmds_subtitle, colour = "Nearest basin") +
  theme_bw()
